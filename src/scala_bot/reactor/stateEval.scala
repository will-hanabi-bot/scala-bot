...
def advance(game: Game, offset: Int): Double =
	...
	val bobChop = Reactor.chop(game, bob)
		...
		urgentDc.orElse(trash.headOption) match {
			case None =>
				...
				if (state.clueTokens > 2)
					...
					clueProb * advance(clueGame, offset + 1) + (1.0 - clueProb) * advance(dcGame, offset + 1)
				else
					Log.info(s"${state.names(playerIndex)} discarding ${state.logId(id)}")
					advance(dcGame, offset + 1)
...
def evalAction(game: Game, action: Action): Double =
	...
	if (value == -100)
		...
		val best = value + advance(hypoGame, 1)
